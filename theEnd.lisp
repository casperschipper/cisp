...
SynthDef(\voice, { | freq = 200, amp = 0.1, number = 3 |
	...
	var voice = SinOsc.ar( freq*a ,0.25, ampEnv*amp ).sum * BPF.ar( WhiteNoise.ar(), 100,0.5,30);
	Out.ar( 0, PanAz.ar(numChans:21,in: (voice*0.1).sin* 0.1, pos:\pan.ir(0)) );
}).send;
)
