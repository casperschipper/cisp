...
class MidiNoteChannelSyncDropStream(EventGenerator):
    "this will also do deffered parameters"
    chuckTemplate = Template("""

function void $funcName() {
    MidiNoteChannelSyncDropStream s;
    $deferedPars
    s.pitch($pitch);
    s.dura($dura);
    s.velo($velo);
...
def standard_env():
    ...
    env.update({
        ...
        'midi-note-channel-sync-drop' : {'name' : 'midi-note-channel-drop', 'args': 5, 'class' : MidiNoteChannelSyncDropStream}
